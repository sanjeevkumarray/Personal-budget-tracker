...
export default function YearToDateReport() {
    const monthNames = [
    ...
    const filteredTxn = allTransactions.filter(
        (singleTxn) => new Date(singleTxn.date).getMonth() <= monthNames.indexOf(month)
    );
    ...
    const charData = [
        { month: `January - ${month}`, income: totalIncome, expense: totalExpense, saving: totalIncome - totalExpense }
    ];

    return (
        <div className="mt-10 max-w-full mx-auto p-6 bg-gray-900 shadow-lg rounded-2xl text-gray-200">
            <h2 className="text-2xl font-semibold mb-4 text-center text-gray-100">📊 Yearly Financial Summary</h2>

            <div className='flex justify-center flex-col lg:flex-row gap-8'>

                <div>
                    <div className="mt-6 flex gap-4 items-center mb-4">
                        <label className="text-gray-300 font-medium">January to:</label>
                        <select 
                            value={month} 
                        ...
                        <table className="w-full text-center">
                            ...
                            <tbody>
                                <tr className="border-b border-gray-700">
                                    ...
                                    <td className={`p-3 font-semibold ${totalIncome > totalExpense ? 'text-green-400' : 'text-red-400'}`}>
                ...
                <div className="p-4 bg-gray-800 rounded-lg w-full md:w-3/4 lg:w-1/2 mx-auto shadow-md">
                    <h3 className="text-xl font-semibold text-gray-100 mb-4 text-center">January - {month} Financial Overview</h3>
